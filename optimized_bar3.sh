...
mem() {
    read -r total used free shared buff cache available < <(awk '/MemTotal/ {total=$2} /MemAvailable/ {available=$2} /Buffers/ {buff=$2} /Cached/ {cache=$2} END {print total, total-available}' /proc/meminfo)
